...
- 由於尚未到達確任點的交易，所有得更新操作都不會影響到資料庫，因此若再確認點之前發生錯誤則不需要做Undo的動作，直接忽略即可
- 到達確認點的交易則需要做Redo，以確認真正改變資料庫
- No Undo/Redo演算法
    1. 交易開始時，將交易記載於Undo串列
    2. 某交易Commit時，將此交易自Undo串列移到Redo串列
    3. 到達Check point時，清空Redo串列
    - 系統發生錯誤後，Undo串列的交易被忽略取消，並不做Undo公做
    - 系統發生錯誤後，從Redo取出以確認的交易並重新執行此串的寫入動作
- 缺點
    - 當系統日誌內的資料量龐大時，十分浪費時間因為Redo動作費時。
    - 經常進行一些不必要的動作，例如大部分得更新都已經寫入資料庫，確一直執行Redo，可結合Checkpoint Checkpoint以減少或解決此一問題

# 立即更新
- 當交易發出一個資料更新(Write)的命令時，位於硬碟上的系統日誌或資料庫可以立即被強至更新，不需要等待交易到達確認點
- 若交易在位到達確認點前失敗，需進行Undo的動作，復原到先前的狀態
- 類型：
    - Undo/Redo演算法：被強至寫入系統日誌
    - Undo/No-Redo演算法:被強至寫入資料庫
- 程序：
    1. 某交易開始時，記載於Undo串列
